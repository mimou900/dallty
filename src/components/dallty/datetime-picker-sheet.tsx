...
/**
 * Full-screen date/time picker — a real calendar with today/tomorrow shortcuts and
 * morning/afternoon/evening period filters. There is no cross-business time-slot
 * inventory in Dallty's backend (availability is only known per business, inside its
 * own booking flow), so this deliberately stays at date + period granularity rather
 * than fabricating individual appointment times the marketplace search can't actually
 * back up.
 */
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-28">
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <p className="font-bold">{monthLabel(viewMonth)}</p>
          <button
            type="button"
            onClick={() =>
              setViewMonth(new Date(viewMonth.getFullYear(), viewMonth.getMonth() + 1, 1))
        ...
        <p className="mt-6 text-sm font-bold">Select a time</p>
          ...
          {PERIODS.map((p) => {
            ...
            return (
              <button
                key={p.key}
                type="button"
                onClick={() => setPeriod(active ? null : p.key)}
                className={`press shrink-0 rounded-2xl border px-4 py-2.5 text-start ${
                  active ? "border-primary bg-primary/8" : "border-border bg-card"
                }`}
              >
      ...
      <div
        className="flex shrink-0 gap-3 border-t border-border/60 bg-cream p-4"
      ...
      >
        ...
        <button
          type="button"
          onClick={() => {
            onApply({ date: selectedDate, period });
            onClose();
          }}
