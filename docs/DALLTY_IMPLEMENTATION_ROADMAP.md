...
| 13 | Affiliate / Referral | **DONE (foundation), NOT STARTED (subscription-triggered activation)** | Confirmed genuinely zero through Project 11. **Built by Project 12** (`project-12-financial-ledger` branch, code complete but NOT yet merged to `main`/deployed as of this writing, 2026-08-21): `affiliates` (auto-approved on apply), `affiliate_commission_rules` (mirrors the existing `commission_rules` precedence hierarchy), `affiliate_referrals` (attribution tracking), a distinct one-off `business_referrals` model, and a new `affiliate_payable` ledger account type. "Becomes a paying subscriber" has no real trigger yet — no subscription system exists (Project 13's job) — so activation is Super-Admin-callable manually as a documented stand-in. See `DALLTY_FINANCIAL_ARCHITECTURE.md` (Project 12 update section). |
