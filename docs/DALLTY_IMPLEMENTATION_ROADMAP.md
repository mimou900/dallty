...
| 06 | Business / Branches / Owners / Staff | **PARTIAL → DONE (branches)** | Business, staff, staff scheduling built and working since earlier projects. `business_branches` existed but stood alone (Project 01) until the booking-engine-branches-foundation work, 2026-08-19: branches are now wired into staff assignment (`staff_branches`), hours (`branch_hours`/`staff_branch_schedules`/`staff_branch_day_hours`), services (`branch_services`), holidays, and bookings — every booking resolves to a specific branch. No membership table still — ownership remains a single column, genuinely NOT STARTED. See `DALLTY_BOOKING_ENGINE.md`. |
| 07 | Services / Availability | **PARTIAL → DONE (branch-aware)** | Services, staff-service assignment, `get_available_slots` built and working since earlier projects; the booking-engine-branches-foundation work, 2026-08-19, rewrote the availability engine onto branch-scoped tables (branch hours ∩ staff hours at that branch, minus breaks/time-off/holidays/blocks, correct multi-interval and timezone math) and closed a real gap where per-staff price/duration overrides were dead — they're now the first-read tier. `branch_services` price/duration overrides exist in schema but aren't yet read by `createBookingHold`'s resolution path — documented gap. See `DALLTY_BOOKING_ENGINE.md`. |
| 08 | Booking Engine | **DONE (backend), PARTIAL (frontend)** | Project 05, 2026-08-17: real concurrency-safe overlap prevention (`EXCLUDE` constraint), live-tested with concurrent requests — no double booking, verified not assumed. Server-side 15-min holds, multi-service duration/price resolution, any-specialist selection, atomic reschedule, `booking_items` historical snapshots. Extended by the booking-engine-branches-foundation work, 2026-08-19: branch-aware everywhere above; a third state machine (`confirmation_status`, pre-appointment call tracking); booking reference codes; configurable hold duration; `createWalkInBooking` (any staff, any specialist, guest or existing customer); a customer branch-picker (auto-skips for the single-branch businesses that are 100% of real data today); a re-run 12-way concurrency test against the live database; a security pass that found and fixed a live RPC-authorization exposure the branch-engine rewrite had introduced. The customer-facing UI is still deliberately NOT rewired onto the hold flow (scope decision, re-confirmed both passes) — it keeps its direct-confirm flow, which inherits the same overlap protection with zero code change. See `DALLTY_BOOKING_ENGINE.md`. |
| 09 | Payments / Financials | **DONE (foundation + payout/deposit/reconciliation/subscription billing), NOT STARTED (gateway)** | Project 06, 2026-08-17. Immutable double-entry ledger, cash settlement with overage/underage reconciliation, commission, staff-earning accrual, refunds — all built and live-tested. No real payment gateway integrated (no credentials exist) — `PaymentProvider` abstraction is ready for one. **Extended by Project 12 (8 phases, `project-12-financial-ledger` branch, code complete but NOT yet merged to `main`/deployed as of this writing):** staff-payout create→settle recording (closes this row's own prior gap), deposit-collection recording, no-show financial policy, a payments-vs-ledger reconciliation tool, country-specific payout field config (Algeria seeded), financial-dashboard filters, and a live security fix (`has_permission()`'s `'self'`-scope gap) plus a 9/9 RLS test matrix pass. **Extended by Project 13 (4 commits, `project-13-subscriptions` branch, code complete but NOT yet merged to `main`/deployed as of this writing):** the real plan/pricing/subscription/billing chain (editable `subscription_plans`, `business_subscriptions` + `subscription_events`), a real `pg_cron` daily lifecycle sweep, a TEMPORARY Super-Admin-only manual payment-recording mechanism (`recordSubscriptionPayment`, reuses Project 12's ledger exactly), real `staff_limit` entitlement enforcement, and two live-testing-driven security fixes (`has_permission()`'s exploitable `'self'`-scope gap in booking actions; `owns_business()`'s manager over-grant on subscription functions) plus an 8/8 RLS test matrix pass. See `DALLTY_FINANCIAL_ARCHITECTURE.md` (Project 12 and Project 13 update sections). |
...
| 13 | Affiliate / Referral | **DONE (foundation + subscription-triggered activation)** | Confirmed genuinely zero through Project 11. **Built by Project 12** (`project-12-financial-ledger` branch, code complete but NOT yet merged to `main`/deployed as of this writing, 2026-08-21): `affiliates` (auto-approved on apply), `affiliate_commission_rules` (mirrors the existing `commission_rules` precedence hierarchy), `affiliate_referrals` (attribution tracking), a distinct one-off `business_referrals` model, and a new `affiliate_payable` ledger account type. "Becomes a paying subscriber" had no real trigger yet — no subscription system existed (Project 13's job). **Closed by Project 13** (`project-13-subscriptions` branch, code complete but NOT yet merged to `main`/deployed as of this writing, 2026-08-22): on a business's first successful `recordSubscriptionPayment`, `activateAffiliateReferralCore`/`activateBusinessReferralCore` (extracted from Project 12's own manual functions so both paths share one implementation) now fire automatically — the business-referral reward is a Super-Admin-editable percentage (`subscription_settings`), not hardcoded. The Super-Admin manual activation path from Project 12 remains available alongside the automatic trigger. See `DALLTY_FINANCIAL_ARCHITECTURE.md` (Project 12 and Project 13 update sections). |
...
| 24 | Production Hardening | **PARTIAL** | 2026-08-18: full Lovable removal — zero runtime dependency on Lovable's email/OAuth/Maps-proxy/asset-CDN infrastructure, deployment retargeted from a silent Cloudflare-Workers default to Vercel's own Nitro preset. See `DALLTY_VENDOR_INDEPENDENCE.md`. Still no CI at all — recommend standing up a minimal CI (lint + typecheck + build) opportunistically, not waiting for a dedicated late-stage phase. |
...
8. Everything else follows the phase table above in roughly its existing order, with
   ...
   layer is.
9. ~~Project 12 — Central Financial Ledger + Revenue + Commissions + Payouts~~ **DONE**
   (8 phases) on branch `project-12-financial-ledger`, 2026-08-21 — staff-payout recording,
   deposit collection, no-show policy, reconciliation, affiliate + business-referral
   foundations, country payout config, dashboard filters, and a live security fix. **Code
   complete on branch, pending merge/deploy — not yet on `main`.** See
   `DALLTY_FINANCIAL_ARCHITECTURE.md` and `DALLTY_MASTER_ARCHITECTURE.md`'s Project 12 entry.
10. ~~Project 13 — Subscription/Billing Architecture~~ **DONE** (4 commits) on branch
    `project-13-subscriptions`, 2026-08-22 — editable plan configuration, the real plan/
    subscription/billing-period/upgrade/downgrade/cancellation/renewal/expiration chain, a
    real `pg_cron` daily lifecycle sweep, a TEMPORARY Super-Admin-only manual payment-recording
    mechanism reusing Project 12's ledger exactly, automatic affiliate/business-referral
    activation on first payment, real `staff_limit` entitlement enforcement, and two
    live-testing-driven security fixes (`has_permission()`'s exploitable `'self'`-scope gap;
    `owns_business()`'s manager over-grant on subscription functions). **Code complete on
    branch, pending merge/deploy — not yet on `main`.** See `DALLTY_FINANCIAL_ARCHITECTURE.md`
    and `DALLTY_MASTER_ARCHITECTURE.md`'s Project 13 entry.
