...
- **RPC function names change; RPC parameter names do not.** Task 3 renames every `owns_salon`/`is_salon_staff`/etc. function itself (e.g. `owns_salon` → `owns_business`), but **discovered live while applying Task 3**: `CREATE OR REPLACE FUNCTION` refuses to rename a parameter or a `RETURNS TABLE` output column ("cannot change name of ... existing function") — the only way to actually rename one is `DROP FUNCTION` + `CREATE FUNCTION`, which is unsafe here (`owns_salon` alone has 41 RLS policies with a hard `pg_depend` dependency on it, confirmed live — a `DROP` would cascade into dropping production policies). Every function's parameters therefore keep their original names (`_salon_id`, `_user_id`, `_staff_id`, etc.), and `get_business_public_staff`'s output column stays `salon_id` too. Every `.rpc("owns_salon", {...})`-style call site (Task 14) changes only the function-name string argument — the parameter object's keys are unchanged.
...
**Interfaces:**
- Produces: functions `owns_business(_user_id uuid, _salon_id uuid)`, `is_business_staff(_user_id uuid, _staff_id uuid)`, `get_business_availability_summary(_salon_id uuid, _days integer)`, `get_business_public_staff(_salon_id uuid)` (returns a `salon_id` column), `submit_business_for_review(_salon_id uuid)`, `recompute_business_listing(_salon_id uuid)`, `tg_recompute_listing_from_business_row()`, `guard_business_marketplace()`, `refresh_business_rating()` — all renamed from their `*_salon*` originals; parameter/output-column names are unchanged (see the correction note after the SQL block — Postgres won't let `CREATE OR REPLACE` rename them, and `DROP`+`CREATE` was unsafe given 41 live policy dependencies on `owns_salon`). Bodies of `check_promo_code`, `get_marketplace_readiness`, `get_available_slots`, `tg_recompute_listing_from_link`, `handle_new_user` rewritten in place (same name, new internal references, `_salon_id` param unchanged).
...
> **Correction, applied at execution time:** the SQL block above (as originally written in this plan) renamed every function's parameters and `get_business_public_staff`'s output column to `_business_id`/`business_id`. Applying it to the live DB failed twice: `CREATE OR REPLACE FUNCTION` refuses to rename a parameter or a `RETURNS TABLE` output column ("cannot change name of ... existing function") when the function itself already exists under that name (whether pre-existing or just renamed via `ALTER FUNCTION RENAME` — renaming the function doesn't rename its parameters). `DROP FUNCTION` + `CREATE FUNCTION` was checked and rejected as unsafe: `owns_salon` alone has 41 RLS policies with a hard `pg_depend` dependency on it, confirmed live, so a `DROP` risks cascading into dropped production policies. **The actual committed migration** — `supabase/migrations/20260814030000_business_rename_functions.sql` — keeps every parameter and `get_business_public_staff`'s output column named `_salon_id`/`salon_id` etc.; only the function *names* and body table/column references changed. Treat that file, not the block above, as the source of truth — every later task in this plan (9, 13, 14) that references an RPC call's parameter keys has been corrected to match it.
...
- `salon_id` → `business_id` for every raw Supabase filter/column reference on a table (e.g. `.eq("salon_id", ...)`) — but **RPC parameter object keys do NOT change** (see Task 3's correction note: none of these functions' parameters were actually renamed by the DB migration; only the function names changed). Also leave alone any `.salon_id` read off an RPC *result* row from `get_business_public_staff` — that function's output column is still named `salon_id` (same Postgres constraint).
- `.from("salons")` → `.from("businesses")`, `.from("salon_gallery")` → `.from("business_gallery")`, `.from("salon_hours")` → `.from("business_hours")`
- `.rpc("get_salon_public_staff", { _salon_id: ... })` → `.rpc("get_business_public_staff", { _salon_id: ... })`, `.rpc("get_salon_availability_summary", { _salon_id: ... })` → `.rpc("get_business_availability_summary", { _salon_id: ... })`, `.rpc("submit_salon_for_review", { _salon_id: ... })` → `.rpc("submit_business_for_review", { _salon_id: ... })` (this one lives in `admin/marketplace.tsx`, not this file — listed here only as a reminder it's part of the same RPC-rename family covered in Task 14) — in every case only the function-name string argument changes, the `_salon_id` key stays. `.rpc("check_promo_code", { _salon_id: ... })` is unchanged entirely (name and param both stay).
...
export async function assertCanManageBusiness(
  ...
  const [{ data: owns }, { data: isAdmin }] = await Promise.all([
    // owns_business's parameter is still named _salon_id (see Task 3's
    // correction note — Postgres won't let CREATE OR REPLACE rename it).
...
5. **RPC calls: rename the function-name string only — every parameter key stays exactly as it was.** Per Task 3's correction note, none of the renamed functions' parameters changed names (Postgres wouldn't allow it, and `DROP`+`CREATE` was unsafe against 41 live policy dependencies). So: `.rpc("owns_salon", { _user_id, _salon_id })` → `.rpc("owns_business", { _user_id, _salon_id })`; `.rpc("is_salon_staff", {...})` → `.rpc("is_business_staff", {...})` (params unchanged: `_user_id, _staff_id`); `.rpc("get_salon_availability_summary", {...})` → `.rpc("get_business_availability_summary", {...})` (params unchanged: `_salon_id, _days`); `.rpc("get_salon_public_staff", {...})` → `.rpc("get_business_public_staff", {...})` (param unchanged: `_salon_id` — and its **result rows** have a `salon_id` field, also unchanged: don't rename `.salon_id` when reading a field off this RPC's return value, only when it's a raw table column); `.rpc("submit_salon_for_review", {...})` → `.rpc("submit_business_for_review", {...})` (param unchanged: `_salon_id`); `.rpc("check_promo_code", {...})` and `.rpc("get_marketplace_readiness", {...})` keep both their function name AND their `_salon_id` param unchanged (these two functions were never renamed, only their bodies were rewritten); `.rpc("get_available_slots", {...})` is unaffected (no salon-named args, name unchanged).
...
`check_promo_code` (line 466) keeps both its name and its `_salon_id` parameter unchanged (per Task 3's correction note) — no edit needed there. Rename every other `salon_id`/`salonId` reference in the file per the rule above.
...
**Correction, applied at execution time:** Steps 1-10's explicit line lists undercounted the actual owner-facing "salon" copy in the repo (the pre-plan audit that produced those line numbers was taken before Tasks 9-14 landed and missed several files/strings entirely — `src/routes/_authenticated/admin/{appointments,customers,index,marketplace,my-appointments,notifications,payments,reports,reviews,route,services}.tsx`, `src/routes/_authenticated/{bookings,dashboard}.tsx`, `src/routes/{auth,business/signup,staff/signup}.tsx`, and `src/components/admin/searchable-select.tsx`). Rather than leave the Step 12 grep failing, every remaining 2nd-person/owner-facing "salon" string (meta descriptions, empty states, button labels, wizard copy, local variable/query-key names built from those strings) was renamed to "business" using the same judgment call as the explicit steps above. Left untouched, per the same rule applied throughout Tasks 9-14: asset file paths/imports (`/salons/hair.jpg`, `salon-hair.jpg`), RPC parameter names (`_salon_id`), audit-log action/resource-type string literals (`"salon"`, `"salon.verify"`), the `assertManagesSalon`/`setSalonVerified` function names and the `salonName` email-template prop (all documented Task 3/13 deviations), example/placeholder text (`layla@salon.com`), and genuinely aggregate marketing/customer-facing copy that was already explicitly kept elsewhere (`dallty-content.ts`, `search.tsx`, `index.tsx` hero/meta copy, `favorites.tsx`, `profile.tsx`, `__root.tsx`, `business-schema.ts`'s "Beauty salon" category value and plan tagline, `email-templates/signup.tsx`). The Step 12 grep now returns only the redirect shim and the three aggregate-copy lines, matching the original expectation.
